...
class ENV():
    def __init__(self, map_path, density, num_amrs, max_steps, planner, workers, cache_db_path, task_mode, scen_path, seed):
        ...
        self.detour_amrs = {aid: 0 for aid in self.amr_list.keys()}
    ...
    def _spawn_amrs_from_task_gen(self):
        ...
        for task in new_tasks:
            ...
            new_amr = AMR(amr_id, start_pos, goal_pos, self.get_task_color(amr_id, goal_pos))
    ...
    def get_task_color(self, amr_id, goal_pos=None):
        if amr_id in self.amr_list:
            return self.amr_list[amr_id].color

        target_dir = self._target_direction_for_color(goal_pos)
        if target_dir is not None:
            return self.cross_1_target_colors[target_dir]

        return self.color_map[amr_id % 6]


    def _target_direction_for_color(self, goal_pos):
        if self.map_name != "cross_1" or goal_pos is None:
            return None
    ...
    def make_info(self):
        ...
        detour_amrs = [aid for aid in self.detour_amrs.keys() if self.detour_amrs[aid] > 0]
        ...
        return {
            ...
            "time": self.time,
            "detour_ratio": len(detour_amrs) / self.num_amrs,
        }
