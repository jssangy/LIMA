...
class DeadlockDetector:
    ...
    def check_deadlock(self, intersection):
        ...
        """
        agvs_inside = list(intersection.agvs_in_intersection.keys())
        ...
        for agv1_num, agv2_num in combinations(agvs_inside, 2):
            if self._check_swapping_path(agv1_num, agv2_num, intersection) or \
                self._check_swapping_path(agv2_num, agv1_num, intersection):
                 return True
            
        return False
    
    def _check_swapping_path(self, agv1_num, agv2_num, intersection):
        """
        A의 경로에 B의 현재 위치가 있는지 확인하고, 경로를 뒤집어
        B의 경로에 포함되는지 검사합니다.
        """
        # 1. 정보 가져오기
        path1 = self.controller.agv_path[agv1_num]
        pos2 = intersection.agvs_in_intersection[agv2_num]
        path2 = self.controller.agv_path[agv2_num]
